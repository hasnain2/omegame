  ...
  return (
    <View style={{backgroundColor: 'black', flex: 1}}>
      {!state.loading && userProfileData?.media.length < 1 ? (
        ...
        <AppPostsListingsGrid navigation={navigation} data={userProfileData.media} style={{backgroundColor: 'black'}} />
      )}
      {state.loading && userProfileData?.media.length < 1 ? <AppLoadingView /> : null}
    </View>
  );
};
