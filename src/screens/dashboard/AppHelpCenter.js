      ...
      <KeyboardAvoidingScrollView>
        <FlatList
          ...
          renderItem={({item, index}) => (
            <List.Section title={item?.section} titleStyle={{color: 'grey', fontWeight: 'bold', fontSize: RFValue(14)}}>
              <FlatList
                data={item?.data || []}
                renderItem={(props) => (
                  <List.Accordion titleStyle={{ color: 'white', fontSize: RFValue(16)}} title={props?.item?.question}>
                    <List.Item
                      title={props?.item?.answer}
                      titleNumberOfLines={20}
                      titleStyle={{color: 'grey', fontSize: RFValue(16), lineHeight: RFValue(22), maxWidth: '95%'}}
                    />
                  </List.Accordion>
                )}
              />
