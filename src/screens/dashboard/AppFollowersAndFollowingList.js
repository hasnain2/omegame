...
const AppFollowersAndFollowingList = ({navigation, route}) => {
  ...
  function handleActionsOnUsers(isUserIsSame, item, index) {
    ...
    if (isFollowerMode && isUserIsSame) {
      // remove follower
      ...
      ActionsOnUsers(
        ...
        item?._id,
        item?.isFollowing ? FRIEND_STATUSES_ACTIONS.UNFOLLOW : FRIEND_STATUSES_ACTIONS.FOLLOW,
      );
      tempData[index] = {
        ...item,
        isRequested: tempData[index]?.isFollowing ? false : true,
        isFollowing: !tempData[index]?.isFollowing,
      };
      if (item?.isFollowing) dispatch(setUser({following: user.following - 1}));
    ...
    <View style={{flex: 1, backgroundColor: 'black'}}>
      ...
      <FlatList
        ...
        renderItem={({item, index}) => {
          let personalizedActionType =
            isFollowerMode && isSameUser
              ? 'Remove'
              : item?.isFollowing
              ? 'Unfollow'
              : item?.isRequested
                ...
                <View style={{flex: 1, paddingLeft: RFValue(10)}}>
                  <View style={{flexDirection: 'row', alignItems: 'center'}}>
                    ...
                    <AppText size={1} bold={true} color={AppTheme.colors.primary} style={{paddingLeft: RFValue(5)}}>
                      {largeNumberShortify(item?.level)}
                    </AppText>
                  </View>
                      ...
                      onPress={() => {
                        if (!item?.isRequested) {
                          if (item?.isFollowing || isFollowerMode) {
                            Alert.alert(
                              `${CapitalizeFirstLetter(personalizedActionType)} ${
                                item?.firstName || item?.userName || 'this user'
                              }`,
                              `Are you sure to ${personalizedActionType.toLowerCase()} ${
                                item?.firstName || item?.userName || 'this user'
                              }?`,
                              [
                                {
