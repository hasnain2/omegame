  ...
  return (
    <View style={{flex: 1, backgroundColor: 'black'}}>
        ...
        <LinearGradient colors={COLORS_ARR} style={{flex: 1, justifyContent: 'flex-end', alignItems: 'center'}}>
          ...
          <View style={{flexDirection: 'row', padding: RFValue(15), alignItems: 'center'}}>
            <View style={{flexDirection: 'row', justifyContent: 'center', alignItems: 'center', flex: 0.3}}>
              <AppGoldCoin size={30} />
              <AppText bold={true} size={1} style={{paddingHorizontal: RFValue(10)}}>
                {largeNumberShortify(user?.earnedCoins || 0)}
              </AppText>
            </View>
            <View style={{flex: 0.3}}>
              <ProgressBar
                style={{height: RFValue(12), borderRadius: 3}}
            ...
            <View style={{flexDirection: 'row', justifyContent: 'center', alignItems: 'center', flex: 0.3}}>
              <AppText size={1} bold={true} style={{}}>
                XP {largeNumberShortify(user?.earnedXps || 0) || '0'}/100
              </AppText>
            </View>
          </View>
          <View
      ...
      <View style={{flexDirection: 'row', alignItems: 'center', justifyContent: 'space-around'}}>
        <TouchableOpacity
          onPress={() =>
            navigation.navigate('AppFollowersAndFollowingList', {isFollowerMode: true, userID: user?._id})
          }>
          <View style={{flexDirection: 'row', alignItems: 'center'}}>
            <AppText bold={true} size={2} color={AppTheme.colors.primary}>
              {user?.followers || '0'}{' '}
            </AppText>
        ...
        <TouchableOpacity
          onPress={() =>
            navigation.navigate('AppFollowersAndFollowingList', {isFollowerMode: false, userID: user?._id})
          }>
          <View style={{flexDirection: 'row', alignItems: 'center'}}>
            <AppText bold={true} size={2} color={AppTheme.colors.primary}>
              {user?.following || '0'}{' '}
            ...
            return (
              <TouchableOpacity
                ...
                <View style={{flexDirection: 'row', alignItems: 'center', padding: RFValue(13)}}>
                  <View style={{flex: 0.15, justifyContent: 'center', alignItems: 'center'}}>{ICON()}</View>
                  <AppText color="white" size={2} style={{flex: 1, paddingLeft: RFValue(15)}}>
                    {itm === 'UserSavedPosts'
                      ? 'Saved Posts'
                      : itm === 'AppSettingsScreen'
                      ? 'Account Settings'
                      : itm === 'LeaveFeedBack'
                      ? 'Leave Feedback'
                      : itm === 'UserProfileCustomizeScreen'
                      ? 'Customize'
                      : itm}
                  </AppText>
                </View>
