      ...
      images.forEach((val) => {
        if (val.node.image && val.node.image.uri) {
          ...
          if (val.node.type.includes('video')) {
            // val.node.image.fileSize
            //   { image: { uri: image.path }, ext, type: "video", upload: true }
            if (AppConfig.IS_IOS_DEVICE) {
              ImagesArray.push({
                image: {uri: ORIGINAL_URI, uri2: uri},
                type: 'video',
                oType: 'video/' + ext,
                upload: true,
                duration: val.node.image.playableDuration,
              });
            } else {
              ImagesArray.push({
                image: {uri: ORIGINAL_URI},
                type: 'video',
                oType: val.node.type,
                upload: true,
                duration: val.node.image.playableDuration,
            ...
            ImagesArray.push({
              image: {uri: val.node.image.uri},
              type: val.node.type === 'image' ? 'photo' : 'photo',
              oType: val.node.type,
              upload: true,
            });
          }
    ...
    <View style={{flex: 1}}>
        ...
        <View
          ...
          <View style={{flexDirection: 'row', alignItems: 'center'}}>
            <EvilIcons
              ...
              name="close"
              style={{fontSize: RFValue(30), padding: RFValue(10), color: 'white'}}
            />
      ...
      <View style={styles.covoerPhotoVideoStyle}>
        {coverPhoto?.type === 'video' ? (
          <AppVideoPlayer
            source={{uri: Platform.OS === 'ios' ? coverPhoto.image.uri2 : coverPhoto.image.uri}}
            startPlaying={true}
          />
        ) : (
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                ...
                selectedOne({
                  ...item,
                  uri:
                    Platform.OS === 'ios' ? (item.type === 'video' ? item.image.uri2 : item.image.uri) : item.image.uri,
                  type: item.type,
                });
              }}>
              <View
                ...
                style={[
                  coverPhoto?.image?.uri === item.image?.uri
                    ? {borderWidth: 3, borderColor: AppTheme.colors.primary, margin: 0}
                    : null,
                  {margin: CARD_MARGIN, height: CARD_SIZE, width: CARD_SIZE, overflow: 'hidden'},
                ]}>
                {item.type === 'video' ? (
                  Platform.OS === 'ios' ? (
