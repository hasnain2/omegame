...
const UserAvatar = ({ source, size, onPress, corner, color }) => {
    ...
    }, [source, size, onPress, corner, color])
    return (
            ...
            <View style={{ justifyContent: 'center', alignItems: 'center', height: SIZE, width: SIZE, }}>
                ...
                {corner ?
                    <View style={{ position: 'absolute', top: 0, bottom: 0, right: 0, left: 0 }}>
                        {color ?
                            <Image source={{ uri: corner || '' }} style={[{ height: SIZE, width: SIZE, borderRadius: 100, overflow: 'hidden' }, color ? { tintColor: color } : null]} />
                            :
                            <FastImage source={{ uri: corner || '' }} style={[{ height: SIZE, width: SIZE, borderRadius: 100, overflow: 'hidden' }, color ? { tintColor: color } : null]} />
                        }
                    </View>
                    : null}
            </View>
