    ...
    return (
        <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center' }}>
            ...
            <View style={{ padding: RFValue(15) }}>
                <UserAvatar corner={item?.corner || ''} color={item?.cornerColor} onPress={() => {
                    if (item?._id)
            ...
            <TouchableOpacity activeOpacity={0.9} style={{ flex: 1 }} onPress={() => {
                ...
                <View style={{ flex: 1, justifyContent: 'center' }} >
                    <View style={{ flexDirection: 'row', alignItems: 'center', }}>
                        <AppText bold={true} size={1} color={AppTheme.colors.lightGrey}>{item?.firstName || item?.userName || ""} {item?.lastName}</AppText>
                        <IsUserVerifiedCheck check={item?.isVerified} />
                        <AppText size={1} bold={true} color={AppTheme.colors.primary} style={{ paddingLeft: RFValue(5) }}>{largeNumberShortify(item?.level || item?.xp || 0)}</AppText>
                        {createdAt && <AppText size={1} color={AppTheme.colors.lightGrey}> - {moment(createdAt).fromNow(true)}</AppText>}
                    </View>
