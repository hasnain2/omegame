...
const SetUpUserAndToken = async (data) => {
  ...
  if (UserObj?._id) firebase.messaging().subscribeToTopic(UserObj?._id);
  return await storeData('user', UserObj)
    .then((res) => {
  ...
  fetch(EndPoints.LOGIN, {
    ...
    .then(async ([status, data]) => {
      ...
      if (status === 201 || status === 200) {
        ...
        callback(true);
      } else callback(data?.message?.message === 'Verify email' ? 'verify' : false);
    })
    .catch((error) => {
      callback(false);
    });
    ...
    .then(([status, data]) => {
      ...
      if (status === 201 || status === 200) {
        ...
        if (data?.message?.message === 'Verify email' || data?.message?.response?.message === 'Verify email') {
          ...
          AppShowToast(
            data?.message?.message || data?.message?.response?.message || 'Something went wrong, try again later',
          );
          callback(false);
        }
      }
